...
export function DadosTab({ org, action }: Props) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <div>
        <h3 className="mb-4 text-sm font-semibold text-zinc-300">Informações da empresa</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <input defaultValue={org.slug} readOnly
              className="w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 font-mono text-sm text-zinc-500 cursor-not-allowed" />
          </div>
          <div className="space-y-1.5">
      ...
      <div>
        <h3 className="mb-4 text-sm font-semibold text-zinc-300">Observações internas</h3>
        <textarea name="notes" defaultValue={org.notes ?? ''} rows={4}
          placeholder="Anotações visíveis apenas para o administrador do sistema..."
          className="w-full rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white placeholder-zinc-600 focus:border-zinc-500 focus:outline-none resize-none" />
      </div>
