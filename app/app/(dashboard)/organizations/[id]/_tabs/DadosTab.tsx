...
export function DadosTab({ org, action }: Props) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <div>
        <h3 className="mb-4 text-sm font-semibold text-zinc-400">Informações da empresa</h3>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <input defaultValue={org.slug} readOnly
              className="w-full rounded-md border border-white/[0.1] bg-white/[0.06] px-3 py-2 font-mono text-sm text-zinc-400 cursor-not-allowed" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-zinc-400">CNPJ</label>
            <input name="cnpj" defaultValue={org.cnpj ?? ''}
              placeholder="00.000.000/0000-00"
              className="w-full rounded-md border border-white/[0.1] bg-white/[0.06] px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 focus:border-white/20 focus:ring-0 focus:outline-none" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-zinc-400">Email de cobrança</label>
      ...
      <div>
        <h3 className="mb-4 text-sm font-semibold text-zinc-400">Observações internas</h3>
        <textarea name="notes" defaultValue={org.notes ?? ''} rows={4}
          placeholder="Anotações visíveis apenas para o administrador do sistema..."
          className="w-full rounded-md border border-white/[0.1] bg-white/[0.06] px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 focus:border-white/20 focus:ring-0 focus:outline-none resize-none" />
      </div>
