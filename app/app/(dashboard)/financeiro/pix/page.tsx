  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Cobranças PIX</h1>
          <p className="mt-1 text-sm text-zinc-500">{charges.length} cobrança{charges.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-zinc-600">A receber</p>
          <p className="mt-2 text-2xl font-bold text-yellow-400">
            {totals.pending.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </p>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-zinc-600">Recebido</p>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {charges.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-10 text-center text-zinc-500">Nenhuma cobrança criada.</td></tr>
            ) : charges.map(c => {
              const cfg = STATUS_CONFIG[c.status] ?? STATUS_CONFIG.PENDING
              return (
                <tr key={c.id} className="border-b border-zinc-800 last:border-0 hover:bg-zinc-800/40 transition-colors">
                  <td className="px-4 py-3 text-zinc-300">{c.client.name}</td>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/financeiro/pix/${c.id}`} className="text-xs text-zinc-400 hover:text-white transition-colors">Ver</Link>
                  </td>
                </tr>
              )
            })}
