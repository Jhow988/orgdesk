...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  ...
  EXPIRED:   { label: 'Expirado',   className: 'bg-white/[0.08] text-zinc-400' },
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100">Cobranças PIX</h1>
            ...
            {charges.length === 0 ? (
              ...
              return (
                <tr key={c.id} className="border-b border-white/[0.06] last:border-0 hover:bg-white/[0.03] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/financeiro/pix/${c.id}`} className="text-xs text-zinc-500 hover:text-zinc-200 transition-colors">Ver</Link>
                  </td>
                </tr>
              )
            })}
