...
import {
  ...
  cancelContaReceberAction,
  deleteContaReceberAction,
} from '@/app/actions/conta-receber'
...
type ContaReceber = Awaited<ReturnType<typeof listContasReceberAction>>[number]
...
interface ContasReceberManagerProps {
  contas: ContaReceber[]
  empresas: Empresa[]
...
function isOverdue(conta: ContaReceber): boolean {
...
function getEffectiveStatus(conta: ContaReceber): string {
...
function StatusBadge({ status }: StatusBadgeProps) {
  const map: Record<string, { cls: string; label: string }> = {
    ...
    RECEIVED: { cls: 'bg-green-900/40 text-green-300 border-green-700', label: 'Recebido' },
...
export function ContasReceberManager({
  ...
  useEffect(() => {
    getCategoriasForSelectAction('RECEITA').then(setCategorias)
  ...
  function applyFilters(status: string, empresaId: string) {
    ...
    router.push(`/financeiro/contas-receber?${params.toString()}`)
  }
  ...
  function handleCreate(formData: FormData) {
    startTransition(async () => {
      const res = await createContaReceberAction(null, formData)
  ...
  function handleUpdate() {
    if (!editing) return
    startTransition(async () => {
      const res = await updateContaReceberAction(editing.id, {
        title: form.title,
  ...
  function handleMarkReceived(id: string) {
    if (!confirm('Marcar esta conta como recebida?')) return
    startTransition(async () => {
      await markContaReceberReceivedAction(id)
      router.refresh()
    })
  }
  ...
  function handleCancel(id: string) {
    ...
    startTransition(async () => {
      await cancelContaReceberAction(id)
      router.refresh()
    })
  }
  ...
  function handleDelete(id: string, title: string) {
    ...
    startTransition(async () => {
      await deleteContaReceberAction(id)
      router.refresh()
    })
  }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-green-800/50 bg-green-900/10 p-4">
          ...
          <p className="mt-1 text-lg font-semibold text-green-300">{formatBRL(totalReceived)}</p>
        </div>
      ...
      {isFormOpen && (
        <div className="rounded-lg border border-zinc-700 bg-zinc-800/60 p-5 space-y-4">
          ...
          >
            ...
            {/* Payer doc */}
            <div>
              <label className={labelCls}>CNPJ / CPF do pagador</label>
              <input
                name="payer_doc"
            ...
            {/* Payment method */}
            <div>
              <label className={labelCls}>Forma de Recebimento</label>
              <select
                name="payment_method"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-700 bg-zinc-800/60">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-zinc-500">Pagador / Cliente</th>
            ...
            <tbody className="divide-y divide-zinc-800">
              {contas.map(conta => {
                ...
                const payerDisplay = conta.payer_name ?? conta.client?.name ?? null
                return (
                  <>
                    <tr key={conta.id} className="bg-zinc-900/40 hover:bg-zinc-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-zinc-400">
                        {payerDisplay ?? <span className="text-zinc-600">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          {(eff === 'PENDING' || eff === 'OVERDUE') && (
                            <button
                              onClick={() => handleMarkReceived(conta.id)}
                              disabled={isPending}
                      ...
                      <tr key={`${conta.id}-detail`} className="bg-zinc-800/20">
                        <td colSpan={7} className="px-6 py-4">
                          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs sm:grid-cols-4">
                            ...
                            {conta.payer_doc && (
                              <div>
                                <span className="text-zinc-500">Doc pagador: </span>
                                <span className="text-zinc-300">{conta.payer_doc}</span>
                              </div>
                            )}
                            ...
                            {conta.received_at && (
                              <div>
                                ...
                                <span className="text-zinc-300">{formatDate(conta.received_at)}</span>
                              </div>
                            )}
                            {conta.received_amount != null && (
                              <div>
                                <span className="text-zinc-500">Valor recebido: </span>
                                <span className="text-zinc-300">{formatBRL(conta.received_amount)}</span>
                              </div>
                            )}
                            {conta.notes && (
                              <div className="sm:col-span-4">
