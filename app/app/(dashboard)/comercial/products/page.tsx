          ...
          <tbody>
            {products.length === 0 ? (
              ...
              <tr key={p.id} className="border-b border-white/[0.06] last:border-0 hover:bg-white/[0.03] transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${p.is_active ? 'bg-emerald-900/50 text-emerald-400' : 'bg-white/[0.08] text-zinc-500'}`}>
                    {p.is_active ? 'Ativo' : 'Inativo'}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <Link href={`/comercial/products/${p.id}`} className="text-xs text-zinc-500 hover:text-zinc-200 transition-colors">Editar</Link>
