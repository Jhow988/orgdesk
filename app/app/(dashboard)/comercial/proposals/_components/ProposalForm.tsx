      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addItem}
            className="rounded-md border border-white/[0.1] px-3 py-1.5 text-xs text-zinc-400 hover:border-white/20 hover:text-zinc-200 transition-colors">
            + Adicionar item
          </button>
        </div>

        {items.length === 0 && (
          <p className="py-4 text-center text-sm text-zinc-500">Nenhum item adicionado. Clique em "+ Adicionar item".</p>
        )}

        {items.map((item, idx) => (
          <div key={item.id} className="rounded-lg border border-white/[0.08] bg-white/[0.04] p-4 space-y-3">
      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-5">
        <div className="flex flex-col items-end gap-2 text-sm">
          <div className="flex gap-6">
            ...
            <span className="font-mono text-zinc-100 w-32 text-right">
          ...
          <div className="flex gap-6 border-t border-white/[0.08] pt-2">
            <span className="font-semibold text-zinc-400">Total</span>
            <span className="font-mono font-bold text-zinc-100 w-32 text-right">
